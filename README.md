...
Excluded by construction are contact details and raw application answers,
`note_file` paths into somebody's Drive, and the names of people who applied and
have not been assigned to a workgroup. The site shows counts per workgroup
instead. `--include-applicant-names` overrides that, and you should only pass it
if the form told applicants their names would be public. Someone who ran a
meeting or owns an action item is published by name, because that is public
organizing work; somebody who filled in a form last week is not.
